...
def _load_token(token):
    """Load current stable tokens and accept the immediately preceding format.

    This lets links generated by the previous deployment finish verification
    instead of forcing every pending seller to restart.
    """
    try:
...
def _verify_seller_whatsapp_fixed(token):
    """Verify Seller Mode directly from the 5-minute signed link."""
    try:
        ...
        if user.role == 'seller' and user.seller_verified:
            # Safe to show success if this exact account has already been verified.
        ...
        # Return success directly. This avoids a second endpoint lookup/redirect
        # being able to turn a successful database commit into a false error.
...
# Keep the legacy success endpoint available for any existing links/bookmarks.
