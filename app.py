...
def _emailjs_config(template_id=None):
    return {
        'service_id': os.environ.get('EMAILJS_SERVICE_ID', '').strip(),
        'public_key': os.environ.get('EMAILJS_PUBLIC_KEY', '').strip(),
        'private_key': (os.environ.get('EMAILJS_PRIVATE_KEY', '').strip() or os.environ.get('EMAILJS_ACCESS_TOKEN', '').strip()),
...
def _send_emailjs(to_email, subject, message, name='', action_url='', action_text='Open Merco', template_id=None):
    ...
    if not cfg['service_id'] or not cfg['public_key'] or not cfg['template_id']:
    ...
    try:
        ...
        if response.ok:
            ...
            return True
        app.logger.error('EmailJS rejected email to %s: HTTP %s: %s', to_email, response.status_code, body)
        return False
    except requests.RequestException as exc:
        app.logger.error('EmailJS network error for %s: %s', to_email, exc)
        return False


def send_email(to_email, subject, text_body, html_body=None, template_id=None, action_url='', action_text='Open Merco'):
    return _send_emailjs(to_email, subject, text_body, action_url=action_url, action_text=action_text, template_id=template_id)


def send_merco_email(user, subject, message, action_url='', action_text='Open Merco', template_id=None):
    return _send_emailjs(user.email, subject, message, name=getattr(user, 'username', ''), action_url=action_url, action_text=action_text, template_id=template_id)
